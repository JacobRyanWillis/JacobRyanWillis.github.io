...
// Download referenced Directus files into public/images and rewrite
// references to local paths, so the static site is fully self-contained.
const imagesDir = new URL('../public/images/', import.meta.url)
await mkdir(imagesDir, { recursive: true })

async function localizeFile(fileId) {
  ...
  await pipeline(Readable.fromWeb(res.body), createWriteStream(new URL(filename, imagesDir)))
